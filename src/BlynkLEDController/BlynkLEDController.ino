/*************************************************************
 * BlynkLEDController.ino
 * Author: Ramb0t 
 * Date: June 2019
 * DIYElectronics.co.za 
 * https://github.com/DIYElectronicsZA/BlynkLEDController
 * 
 * Description: 
 * Modification of Blynk sample app for NodeMCU, supports 4 channels of PWM
 * to drive (N-Chan MOSFET IRLB8743) Transistors for common anode LED strip 
 * RGBW (or RGB if thats what you have) strip can be dynamically adjusted, 
 * via WiFi using the Blynk App. 
 * A switch input is also used for a manual On/Off. 
 * "On" mode sets the white channel to maximum.
 * Debug information is displayed on the OLED, such as PWM values. 
 * Display times out and blanks after a delay, to prevent OLED burn in. 
 * Arduino ESP8266 OTA updates are also supported, for wireless reflashes
 * after the inital config.  
 * 
 * Remember to rename the 'config_sample.h' file to 'config.h'
 * and enter both your Blynk app token, as well as SSID settings. 
 * 
 * The D-Duino V1 NodeMCU based board, with built in SSD1306 OLED display 
 * is used for this device, however a normal NodeMCU should work too. 
 * Please install this library (avaliable with the Arduino Lib Manager)
 * for the OLED to work. https://github.com/ThingPulse/esp8266-oled-ssd1306
 * 
 */
